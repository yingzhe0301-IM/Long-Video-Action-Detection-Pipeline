...
Core orchestration lives at the repository root: `experiments.py` loads YOLO checkpoints, runs batch processing, and evaluates detections. Supporting modules now sit inside the `haul/` package (for example `haul/processing/batch_processor.py`, `haul/actions/action_registry.py`, and `haul/actions/action_evaluator.py`) to keep batching, heuristics, and scoring concerns organised. Model assets belong in `model_weights/`, demo clips under `video/selected_test_video/`, and generated plots or CSVs in `plot/`. Keep exploratory notebooks (`haul.ipynb`) and staging folders under `video/` (for example `video/new_video/` or `video/test_video/`) isolated from production edits.
